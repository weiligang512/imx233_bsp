...
static void dm9000_rx_status_8bit(u16 *RxStatus, u16 *RxLen)
{
	...
	*RxLen =
	    __le16_to_cpu(DM9000_inb(DM9000_DATA) +
			  (DM9000_inb(DM9000_DATA) << 8));
	*/
	*(u8*)RxStatus = dm9000_read();
	((u8*)RxStatus)[1] = dm9000_read();

	*(u8*)RxLen = dm9000_read();
	((u8*)RxLen)[1] = dm9000_read();
...
static int dm9000_rx(struct eth_device *netdev, void (*NetReceive)(char *, int))	//zhai added callback functions
{
	...
	for (;;) {
		...
		*(u8*)&rxbyte = dm9000_read(); //include dummy read
		rxbyte &= 0x03;
